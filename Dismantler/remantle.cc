...
void apply_effect(fragment::key<G1>& key, effect::Bitmap& source){
	...
	// Run operations
	for(s64 i = operations.size() - 1; i >= 0; --i){
		fragment::operation op = operations[i];
		for(s64 j = op.code.size() - 1; j >= 0; --j){
			// Get current opcode
			size_t mangled_opcode = op.code[j];
